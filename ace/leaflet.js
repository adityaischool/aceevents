var map = L.map('map').setView([37.81, -122.5155], 11);
...
//var grid0 = L.rectangle([grid[0][0], grid[0][3]]).addTo(map);
...
function drawGrid(map, gridCoords, grid) {
	...
	for (var i=0; i < gridCoords.length; i++) {
		...
		rectangle = new L.rectangle([gridCoords[i][0], gridCoords[i][3]]);
