...
function refreshSliderGradient() {

	var gradientList = "";

	$.getJSON('/_refreshSliderGradient', {
		...
		console.log(data);
		//console.log(data['totalCap'].split(","));
		console.log(typeof data['totalCap']);

		console.log(data['totalCap']);

		var stripped1 = data['totalCap'].replace(/\'/g, '');
		stripped2 = stripped1.replace(/\[/g, '');
		stripped3 = stripped2.replace(/\]/g, '');

		console.log(stripped3);

		var split = stripped3.split(',');
		console.log(split);
		//console.log(data['totalCap'][i]);
		//console.log(Number(data['totalCap'][i]));

		//$('.bar').css({"background-image": "linear-gradient(to right,green, red)"});

		for (var i = 0; i < split.length; i ++) {



			var hex = capValueToHex(Number(split[i].trim()));

			console.log(Number(split[i].trim()));
		...
		var insert ="linear-gradient(to right,"+gradientList+")";

		console.log(insert);

		$('.bar').css({"background-image": insert});


})};

function capValueToHex(value) {
...
var child = document.getElementById('eventInfoList');
...
$('#eventInfoList').scroll(function (event) {

	var scroll = $('#eventInfoList').scrollTop();
...
function checkRad(currentLocation,
 destination) {
	var rad = .2;
